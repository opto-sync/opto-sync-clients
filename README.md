...
- [Zed package](docs/ZED_PACKAGE.md) — current whole-repository source package, lockfile status, and language-target migration
...
The `Zed packaging boundaries` job and the separate package workflow run
`scripts/check-zed-packaging.py`. The check preserves the valid root source
package while rejecting isolated language targets until every native engine path
is contained within its target artifact.
