...
`main.tsp` and `authored.schema.json` are independently maintained peer authorities for this clients repository's parity-gate fixture. Neither file is generated from, ranked below, or allowed to overwrite the other.
...
A passing fixture proves this repository executes the fail-closed gate. It does not move Opto Sync product schema ownership into the clients repository and does not certify generated SDKs or downstream wrappers. Canonical product authorities remain in `opto-sync-interfaces`; any unexplained mismatch stops evaluation and blocks promotion.
