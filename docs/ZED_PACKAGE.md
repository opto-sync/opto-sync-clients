...
`opto-sync-clients` is prepared first as the whole-repository Zed source package
`opto-sync/opto-sync-clients@0.2.0`. Do not claim a registry release until the
engine dependency is resolved into the lockfile and the matching Git tag points
...
The whole-repository artifact is a license-complete, reproducible source bundle,
but the native client manifests still expect a sibling Git checkout named
`syncer.c`. Installing the Zed dependency does not rewrite those ecosystem-native
paths or create that sibling alias. Native builds must therefore continue to use
the documented checkout layout until the path contract is migrated.
...
Normal client CI remains authoritative for runtime behavior across Node, real
Chromium IndexedDB/WASM, Dart SQLite/FFI, Rust SQLite, and Gleam/BEAM. It uses an
immutable compatible engine commit by default, removes checkout tokens before
executing build code, and installs Node dependencies through committed locks. A
manual dispatch can test a candidate engine ref explicitly.

`scripts/check-zed-packaging.py` validates the package identity, MIT license,
dependency, lockfile format, and every native path. It permits the current
whole-repository source package but rejects `[targets]` while any required engine
path escapes a target artifact. The Zed workflow runs the same check against the
worktree and extracted tarball, inspects the archive for required manifests and
forbidden cache/build directories, executes the declared smoke boundary, and
uploads the deterministic artifact.
