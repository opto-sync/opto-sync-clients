...
`.zpkg.toml`. For version `0.5.0`, the accepted tag is exactly `v0.5.0`—not an
...
3. provision the `opto-sync` registry namespace and this repository's
   `ZED_PKG_TOKEN`;
4. place `v0.5.0` on the reviewed `main` commit; and
5. let the tag workflow publish `opto-sync/opto-sync-clients@0.5.0`.
