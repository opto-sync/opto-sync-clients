import { DEFAULT_FORM_DATABASE, DEFAULT_FORM_STORE, DEFAULT_MAX_FORM_PAYLOAD_BYTES, DEFAULT_MAX_PENDING_FORMS, FORM_SYNC_STATUS, } from './types.js';
...
export class BrowserFormQueue {
    databaseName;
    maxPending;
    maxBytes;
    database;
    constructor(options = {}) {
        this.databaseName = required(options.databaseName ?? DEFAULT_FORM_DATABASE, 'databaseName');
    ...
    async queueMutation(tableName, recordId, payload, protocol = {}) {
        ...
        try {
            const count = await requestResult(store.index('syncStatus').count(IDBKeyRange.only(FORM_SYNC_STATUS.PENDING)));
            ...
            if (!Number.isSafeInteger(id))
                throw new Error('invalid IndexedDB mutation id');
            return id;
        }
        catch (error) {
            abort(transaction);
    ...
    async pendingMutations(tableName) {
        ...
        const done = transactionDone(transaction);
        const rows = (await requestResult(transaction
    ...
    async markMutation(id, syncStatus) {
        ...
        if (!Object.values(FORM_SYNC_STATUS).includes(syncStatus)) {
        ...
        try {
            ...
            if (!row)
                throw new Error(`form mutation ${id} was not found`);
...
export function createBrowserFormQueue(options) {
