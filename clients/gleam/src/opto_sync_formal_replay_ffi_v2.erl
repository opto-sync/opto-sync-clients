%% Strict OTP-27 JSON replay harness for fmctl.adapter.v1.
%%
%% This module replaces only the operational file/process/ITF traversal shim.
%% Protocol transitions, response validation, queue mutation, acknowledgement,
%% checkpoint changes, and canonical observations remain delegated to the
%% production Gleam modules.
-module(opto_sync_formal_replay_ffi_v2).
-export([main/0]).
...
execute() ->
    Request = object(decode_json(read_stdin()), <<"adapter request">>),
        ...
        required_binary(Request, <<"protocol">>) =:= ?PROTOCOL,
        <<"unsupported adapter protocol">>
    ),
    TracePaths = required_list(Request, <<"tracePaths">>),
    ensure(length(TracePaths) =:= 1, <<"isolated BEAM session requires exactly one trace">>),
    [Path] = TracePaths,
    ensure(is_binary(Path) andalso byte_size(Path) > 0, <<"trace path must be a non-empty string">>),
    StateCount = replay_trace(Path),
    #{
        <<"protocol">> => ?PROTOCOL,
        <<"result">> => #{
            <<"status">> => <<"ok">>,
...
apply_action(Action, TraceState, Adapter) ->
    ensure(
        maps:get(initialized, Adapter),
        format_binary("action ~s occurred before init", [Action])
    ),
    apply_initialized(Action, TraceState, Adapter).

apply_initialized(<<"idle">>, _TraceState, Adapter) -> Adapter;
apply_initialized(<<"compact">>, _TraceState, Adapter) -> Adapter;
...
apply_initialized(<<"send">>, TraceState, Adapter) ->
    ...
    Id = picked_id(TraceState),
        ...
        lists:member(Id, model_set(model_state(TraceState), <<"pending">>)),
        <<"send selected a non-pending mutation">>
    ),
    Adapter#{request := Id};
apply_initialized(<<"apply_new">>, TraceState, Adapter) ->
    server_reply(TraceState, Adapter, true, <<"applied">>, none);
apply_initialized(<<"reject_new">>, TraceState, Adapter) ->
    server_reply(TraceState, Adapter, false, <<"rejected">>, none);
apply_initialized(<<"reply_duplicate">>, TraceState, Adapter) ->
    Model = model_state(TraceState),
    Id = picked_id(TraceState),
    Applied = lists:member(Id, model_set(Model, <<"applied">>)),
    Original = case Applied of true -> <<"applied">>; false -> <<"rejected">> end,
    server_reply(TraceState, Adapter, Applied, <<"duplicate">>, Original);
apply_initialized(<<"inject_mismatched_response">>, TraceState, Adapter) ->
    ...
    Checkpoint = model_bigint(Model, <<"response_checkpoint">>),
        ...
        opto_sync_formal_adapter:mismatched_response_rejected(
            RequestId,
            ResponseId,
            Watermark,
            Checkpoint
        ) =:= true,
        <<"production Gleam validator accepted mismatched response">>
    ),
    Adapter#{response := #{
...
apply_initialized(<<"acknowledge">>, TraceState, Adapter) ->
    Id = picked_id(TraceState),
    ensure(require_request(Adapter) =:= Id, <<"acknowledgement id differs from request">>),
    Response = require_response(Adapter),
...
apply_initialized(<<"begin_reset">>, _TraceState, Adapter) ->
    ensure(not maps:get(replacing, Adapter), <<"snapshot replacement already active">>),
...
apply_initialized(<<"crash_during_reset">>, _TraceState, Adapter) ->
    ensure(maps:get(replacing, Adapter), <<"reset crash without active replacement">>),
...
apply_initialized(<<"finish_reset">>, TraceState, Adapter) ->
    ensure(maps:get(replacing, Adapter), <<"finish reset without active replacement">>),
...
server_reply(TraceState, Adapter, Applied, Status, OriginalStatus) ->
    Id = picked_id(TraceState),
    ensure(require_request(Adapter) =:= Id, <<"server reply id differs from in-flight request">>),
    ensure(maps:get(response, Adapter) =:= none, <<"response already present">>),
...
assert_reply(Reply, Model, Status, OriginalStatus) ->
    ...
    ensure(json_bigint(required(Reply, <<"mutationId">>)) =:= model_bigint(Model, <<"response_mutation_id">>), <<"production reply mutation id differs from model">>),
    ensure(json_bigint(required(Reply, <<"watermark">>)) =:= model_bigint(Model, <<"response_watermark">>), <<"production reply watermark differs from model">>),
    ensure(json_bigint(required(Reply, <<"checkpoint">>)) =:= model_bigint(Model, <<"response_checkpoint">>), <<"production reply checkpoint differs from model">>),
    case OriginalStatus of
        none -> ok;
        Expected -> ensure(required_binary(Reply, <<"originalStatus">>) =:= Expected, <<"duplicate original status differs from model">>)
    end.
...
assert_projection(Adapter, TraceState) ->
    ...
    ensure(json_bigint(required(Actual, <<"nextId">>)) =:= ExpectedNext, <<"next mutation id differs from model">>),
    ensure_equal_set(json_set(required(Actual, <<"pending">>)), model_set(Model, <<"pending">>), <<"pending mutation ids">>),
    ensure_equal_set(json_set(required(Actual, <<"confirmed">>)), model_set(Model, <<"acknowledged">>), <<"confirmed mutation ids">>),
    ensure_equal_set(json_set(required(Actual, <<"allocated">>)), allocated_ids(ExpectedNext), <<"allocated mutation ids">>),
    ensure(json_bigint(required(Actual, <<"checkpoint">>)) =:= model_bigint(Model, <<"local_checkpoint">>), <<"pull checkpoint differs from model">>),
    ExpectedPhase = model_reset_phase(Model),
    ensure(required_binary(Actual, <<"resetPhase">>) =:= ExpectedPhase, <<"production reset phase differs from model">>),
    ensure(maps:get(replacing, Adapter) =:= (ExpectedPhase =:= <<"replacing">>), <<"adapter reset control differs from model">>),
    ensure_equal_outcomes(actual_outcomes(required_list(Actual, <<"knownOutcomes">>)), expected_outcomes(Model)),
    assert_request(Adapter, Model),
    assert_response(Adapter, Model),
    ok.

allocated_ids(1) -> [];
allocated_ids(NextId) when NextId > 1 -> lists:seq(1, NextId - 1).
...
assert_response(Adapter, Model) ->
    ...
    case {Present, maps:get(response, Adapter)} of
        ...
        {true, Response} ->
            ensure(maps:get(mutation_id, Response) =:= model_bigint(Model, <<"response_mutation_id">>), <<"response mutation id differs from model">>),
            ensure(maps:get(watermark, Response) =:= model_bigint(Model, <<"response_watermark">>), <<"response watermark differs from model">>),
            ensure(maps:get(checkpoint, Response) =:= model_bigint(Model, <<"response_checkpoint">>), <<"response checkpoint differs from model">>),
            ensure(maps:get(valid, Response) =:= model_bool(Model, <<"response_valid_for_in_flight">>), <<"response validity differs from model">>)
    end.
...
ensure_equal_outcomes(Actual, Expected) ->
    ensure(Actual =:= Expected, format_binary("known outcomes ~p differ from model ~p", [Actual, Expected])).

ensure_equal_set(Actual, Expected, Label) ->
    Left = lists:usort(Actual),
    Right = lists:usort(Expected),
    ensure(Left =:= Right, format_binary("~s ~p differ from model ~p", [Label, Left, Right])).
...
model_state(TraceState) -> object(required(TraceState, <<"s">>), <<"model state">>).
model_bigint(Model, Key) -> json_bigint(required(Model, Key)).
model_set(Model, Key) -> json_set(required(Model, Key)).
...
json_bigint(Value) when is_integer(Value) -> Value;
json_bigint(Value) ->
    Tagged = object(Value, <<"tagged bigint">>),
    Encoded = required_binary(Tagged, <<"#bigint">>),
...
json_set(Value) ->
    Tagged = object(Value, <<"ITF set">>),
    lists:sort([json_bigint(Item) || Item <- required_list(Tagged, <<"#set">>)]).
...
object(Value, _Label) when is_map(Value) -> Value;
object(_Value, Label) -> fail(format_binary("~s must be an object", [Label])).
...
emit(Value) -> io:put_chars(standard_io, [encode_json(Value), <<"\n">>]).
...
fail(Message) when is_binary(Message) -> throw({adapter_error, Message}).
format_binary(Format, Args) -> iolist_to_binary(io_lib:format(Format, Args)).
