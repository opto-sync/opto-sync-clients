  ...
  case is_directory(candidate), remaining {
    ...
    False, _ -> {
      ...
      case above == directory {
        True ->
          panic as "could not locate schema/fixtures above the filesystem root"
...
pub fn rejects_a_table_identifier_that_is_not_sql_safe_test() {
  ...
  should_reject(envelope_with(
    "{\"table\":\"\",\"recordId\":\"t\",\"payload\":{\"updatedAt\":1}}",
  ))
  should_reject(envelope_with(
...
pub fn rejects_an_invalid_optional_timestamp_test() {
  ...
  should_reject(envelope_with(upsert_with("{\"updatedAt\":1,\"syncedAt\":-1}")))
  should_accept(
...
pub fn rejects_a_record_id_outside_one_to_512_characters_test() {
  should_reject(envelope_with(
    "{\"table\":\"todos\",\"recordId\":\"\",\"payload\":{\"updatedAt\":1}}",
  ))
  should_reject(envelope_with(
