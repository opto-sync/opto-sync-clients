...
fn ingest_queues_one_mutation_per_record_in_file_order() {
    ...
    let raw = read(&locate_fixtures().join("valid").join("nested-keyed-arrays.json"));
    ...
    let payload = pending[0].payload.as_ref().expect("upsert carries a payload");
...
fn nothing_queues_when_any_record_is_invalid() {
    let mut queue = ProtocolQueue::new("rust-fixture-client").unwrap();
    let raw = read(&locate_fixtures().join("invalid").join("missing-updated-at.json"));
