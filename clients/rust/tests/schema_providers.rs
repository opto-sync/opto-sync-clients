...
use opto_sync_client::schema::{
    audit_provider, garde_provider, json_schema_provider, parse_envelope,
    parse_envelope_with, validator_provider,
};
...
fn accepts_unicode_code_point_boundaries() {
    let envelope = parse_envelope(&fixture(
        "valid",
        "safe-integer-unicode-boundaries.json",
    ))
...
fn named_library_adapters_are_veto_gates() {
    ...
    assert!(error.issues.iter().any(|issue| issue.contains("provider[validator]")));
    assert!(error.issues.iter().any(|issue| issue.contains("provider[garde]")));
    assert!(error.issues.iter().any(|issue| issue.contains("provider[jsonschema]")));
}

#[test]
fn audit_reports_acceptance_drift() {
    let value: Value = serde_json::from_str(&fixture("invalid", "null-operation.json"))
