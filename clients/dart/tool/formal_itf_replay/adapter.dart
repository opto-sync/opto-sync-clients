...
Future<Map<String, dynamic>> _responseFromState(
  ...
  final clientId = adapter.request?['clientId'] ?? await adapter.client.clientId();
...
Future<Map<String, Object?>> _databaseSnapshot(_Adapter adapter) async {
  final mutations = await (adapter.db.select(adapter.db.localMutations)
        ..orderBy([(table) => OrderingTerm.asc(table.id)]))
      .get();
  final metadata = await (adapter.db.select(adapter.db.meta)
...
Future<_Adapter> _applyAction(
  ...
  switch (state.action) {
    ...
    case 'enqueue':
      ...
      final row = await (current.db.select(current.db.localMutations)
    ...
    case 'inject_mismatched_response':
      ...
      final response = await _responseFromState(
        current,
        state,
        'applied',
      );
      ...
      _ensure(
        ...
        expected: 'FormatException(push acknowledgement does not match the sent batch)',
